...
// ฟังก์ชันปัดเศษทศนิยมไม่ให้ปัดขึ้น (Truncate to 2 decimals)
...
function calculateLineTotal(index) {
    ...
    // คำนวณ subtotal และ total โดยไม่ปัดเศษขึ้น
    var subtotal = qty * price;
    var total = subtotal - discount;
    
    console.log('Qty:', qty, 'Price:', price, 'Discount:', discount, 'Total:', total);
    
    $('.line-total[data-index="' + index + '"]').val(toFixedNoRound(total));
    calculateGrandTotal();
}
...
function calculateGrandTotal() {
    ...
    discount = parseFloat(toFixedNoRound(discount));
   ...
   // if(purch_req_is_vat === 1 || purch_req_is_vat =='1'){
        ...
        vat = parseFloat(toFixedNoRound(vat));
    //}
    
    total = total + vat;
    total = parseFloat(toFixedNoRound(total));
    
    // console.log('Grand total:', total);
    
    $('#quotation-total_amount').val(toFixedNoRound(total));
    $("#sub-total").html(formatNumber(toFixedNoRound(total_before_discount)));
    $("#discount-total").html(formatNumber(toFixedNoRound(discount)));
    $('#summary-total').text(formatNumber(toFixedNoRound(total)));
    
    $('#sub-total-amount').val(toFixedNoRound(total_before_discount));
    $('#discount-total-amount').val(toFixedNoRound(discount));
    $('#total-vat-amount').val(toFixedNoRound(vat));
    $('#summary-total-amount').val(toFixedNoRound(total));
}
...
// Event handler สำหรับจัดรูปแบบตัวเลขเมื่อเปลี่ยนค่า (on change/blur)
$(document).on('change', '.price-input, .discount-input, #purch-discount_amount', function() {
    var val = parseFloat($(this).val()) || 0;
    $(this).val(toFixedNoRound(val));
});
JS;
