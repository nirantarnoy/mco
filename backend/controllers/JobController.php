...
//    }
    ...
    protected function getPurchaseOrders($jobId)
    {
        $query = "
            SELECT 
                ...
                p.vendor_id,
                p.approve_status,
                (p.total_amount * COALESCE(NULLIF(p.currency_rate, 0), 1)) as total_amount,
                (p.discount_amount * COALESCE(NULLIF(p.currency_rate, 0), 1)) as discount_amount,
                (p.vat_amount * COALESCE(NULLIF(p.currency_rate, 0), 1)) as vat_amount,
    ...
    public function actionPullQuotationDetails()
    {
        ...
        if ($id && $quotation_id) {
            // ลบรายการเดิมออกก่อนเพื่อให้ข้อมูลตรงกับใบเสนอราคาล่าสุด
            \common\models\JobLine::deleteAll(['job_id' => $id]);
