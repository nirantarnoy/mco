        ...
        if (!empty($searchQuery)) {
            ...
            if (!empty($keywords)) {
                ...
                if (class_exists('backend\models\Invoice')) {
                    $q = Invoice::find()->joinWith(['items'])->groupBy('invoices.id');
                    $cond = ['or'];
                    foreach ($keywords as $keyword) {
                        $cond[] = ['like', 'invoices.invoice_number', $keyword]; // Also fixed invoice_no to invoice_number if needed? Wait let's check Invoice model
                    }
                    if (!empty($productIds)) {
